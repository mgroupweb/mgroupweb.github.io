...
3. Secrets (each is optional; the checker shows "—" for providers that are not configured):
   ...
   - `wrangler secret put ALERT_WEBHOOK` — Slack incoming webhook URL (or any Slack-compatible endpoint). The Worker posts a message once per day when the Moz monthly quota is exhausted.
